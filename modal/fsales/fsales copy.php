<?php
	...
	switch($modal){
		...
		$read	= $conn->prepare("SELECT kode_tfk, tgl_tfk, sj_tfk, tglsj_tfk, po_tfk, tglpo_tfk, tgl_limit FROM transaksi_faktur WHERE id_tfk=:kode");
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-6">
                    ...
                    <input type="text" name="nomorfak" class="form-control" value="<?php echo($view['kode_tfk']); ?>" placeholder="Type here..." required="required" />
                </div>
                <div class="form-group col-md-6">
...
<?php
		...
		case 'delete':
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT status_tfk FROM transaksi_faktur WHERE id_tfk=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
