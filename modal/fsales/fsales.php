<?php
	...
	switch($modal){
		...
		$read	= $conn->prepare("SELECT * FROM transaksi_faktur_c WHERE id_tfk=:kode");
...
?>
        ...
        <div class="modal-body">
            ...
            <div class="row">
                ...
                <div class="form-group col-md-6">
                    <label>Status Pajak<span class="tx-danger">*</span></label>
                    <input type="text" name="taxStatus" class="form-control" value="<?php echo($view['pajak_tfk']); ?>" required="required" readonly/>
                </div>
			</div>
        </div>
        ...
        <input type="hidden" name="keycode" value="" readonly="readonly" />
        <div class="modal-body">
...
<?php
		...
		case 'delete':
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT status_tfk, kode_tfk FROM transaksi_faktur_c WHERE id_tfk=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
