		...
		switch($menu){
			case 'faktur':
				...
				try {
                    ...
                    if (is_array($dRead)) {
						array_push($msgBugs, "Nomor PO ".$nomorpo." sudah pernah digunakan!");
					}
				...
				if (empty($msgBugs)) {
					...
					if (empty($msgBugs)) {
						$kode	= $data->transcode($gabung, 'sj_tfk', 'transaksi_faktur_c');
						$nofak	= $data->transcode($gobong, 'kode_tfk', 'transaksi_faktur_c');
						$qSave = "INSERT
									INTO
										transaksi_faktur_c
									VALUES(
			...
			case 'items':
				$uniq	= $secu->injection($_POST['keycode']);
				$taxStatus	= $secu->injection($_POST['taxStatus']);
				$code	= base64_decode($uniq);
				...
				if (empty($msgBugs)) {
					...
					try {
						...
						$edit->bindParam(':taxStatus', $taxStatus, PDO::PARAM_STR);
						$edit->execute();
					} catch (PDOException $e) {
						array_push($msgBugs, $e->getMessage());
					}
			...
			case 'update':
				...
				try {
					$edit	= $conn->prepare("UPDATE transaksi_faktur_c SET kode_tfk=:nofak, tgl_tfk=:tglfak, sj_tfk=:nosj, tglsj_tfk=:tglsj, po_tfk=:nopo, tglpo_tfk=:tglpo, tgl_limit=:jatuh, updated_at=:catat, updated_by=:admin WHERE id_tfk=:kode");
				...
				if (empty($msgBugs)) {
					try {
						$riwayat = $conn->query("INSERT INTO riwayat_c (kode_riwayat, menu_riwayat, status_riwayat, ket_riwayat, created_at, created_by) VALUES('$kode', 'Faktur Penjualan', 'Update', '', '$catat', '$admin')");
					} catch (PDOException $e) {
						array_push($msgBugs, $e->getMessage());
					}
				}
			...
			case 'delete':
				...
				try {
					$master	= $conn->prepare("SELECT id_psd, jumlah_tfd FROM transaksi_fakturdetail_c WHERE id_tfk=:kode");
				...
				*/
				if (empty($msgBugs)) {
					try {
						$remove	= $conn->prepare("DELETE A, B, C FROM transaksi_faktur_c AS A LEFT JOIN transaksi_fakturdetail_c AS B ON A.id_tfk=B.id_tfk LEFT JOIN pembayaran_faktur_c AS C ON A.id_tfk=C.id_tfk WHERE A.id_tfk=:kode");
				...
				if (empty($msgBugs)) {
					try {
						$riwayat= $conn->query("INSERT INTO riwayat_c (kode_riwayat, menu_riwayat, status_riwayat, ket_riwayat, created_at, created_by) VALUES('$kode', 'Faktur Penjualan', 'Delete', '', '$catat', '$admin')");
					} catch (PDOException $e) {
						array_push($msgBugs, $e->getMessage());
					}
				}
