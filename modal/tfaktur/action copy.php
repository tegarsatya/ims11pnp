	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			case "input":
				...
				$save			= $conn->prepare("INSERT INTO tuker_faktur_c VALUES(:kode, :kodetukerfaktur, :namaoutlet, :tglfaktur, :tgltempo, :tanggal_tkf, :status,  :catat, :admin, :catat, :admin)");
			...
			case "update":
				...
				$tanggal_tkf	= $secu->injection($_POST['tanggal_tkf']);
				$status			= $secu->injection($_POST['status']);
				$edit			= $conn->prepare("UPDATE tuker_faktur_c SET id_kot=:id_kot, id_tfk=:id_tfk, tanggal_tkf=:tanggal_tkf, status=:status, updated_at=:catat, updated_by=:admin WHERE id_tkf_t=:kode");
			...
			case "delete":
				$kode	= $secu->injection($_POST['keycode']);
				$remove	= $conn->prepare("DELETE FROM tuker_faktur_c WHERE id_tkf_t=:kode");
