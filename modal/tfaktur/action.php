	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			case "input":
				...
				$save			= $conn->prepare("INSERT INTO tuker_faktur_c VALUES(:kode,:id_kot, :id_tfk,  :tanggal_tkf, :status, :note, :catat, :admin, :catat, :admin)");
			...
			case "update":
				...
				$status			= $secu->injection($_POST['status']);
				$note			= $secu->injection($_POST['note']);
				$edit			= $conn->prepare("UPDATE tuker_faktur_c SET id_kot=:id_kot, id_tfk=:id_tfk, tanggal_tkf=:tanggal_tkf, status=:status, note=:note, updated_at=:catat, updated_by=:admin WHERE id_tkf_t=:kode");
