                ...
                <div class="form-group col-md-12">
                	<table width="100%">
                    	<tr>
                            ...
                            <?php
                                $lunas	= 'Lunas';
                                $master	= $conn->prepare("SELECT A.id_tfk, A.kode_tfk, B.nama_out FROM transaksi_faktur_c AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out WHERE A.status_tfk!=:lunas ORDER BY A.tgl_tfk ASC");
...
<?php
		...
		$read	= $conn->prepare("SELECT A.id_tfk, A.bank_pfk, A.norek_pfk, A.anam_pfk, A.jumlah_pfk, A.tgl_pfk, B.status_tfk, B.total_tfk, B.kode_tfk, C.nama_out FROM pembayaran_faktur_c AS A LEFT JOIN transaksi_faktur_c AS B ON A.id_tfk=B.id_tfk LEFT JOIN outlet AS C ON B.id_out=C.id_out WHERE A.id_pfk=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);

		$mbayar	= $conn->prepare("SELECT SUM(jumlah_pfk) AS total FROM pembayaran_faktur_c WHERE id_tfk=:kode");
...
<?php
		...
		case "detail":
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT kode_tfk, status_tfk FROM transaksi_faktur_c WHERE id_tfk=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
            ...
            <div class="row">
	            <div class="form-group col-md-12">
                    ...
                    <table class="table table-bordered">
                        ...
                        <?php
							$no		= 1;
							$master	= $conn->prepare("SELECT jumlah_pfk, file_pfk, tgl_pfk FROM pembayaran_faktur_c WHERE id_tfk=:kode ORDER BY tgl_pfk DESC");
