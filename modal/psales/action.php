<?php
	...
	switch($act){
		case "input":
			$kode	= $data->basecode('', 1, 'id_pfk', 'pembayaran_faktur_c');
			...
			//EDIT
			$edit	= $conn->prepare("UPDATE transaksi_faktur_c SET status_tfk=:status, updated_at=:catat, updated_by=:admin WHERE id_tfk=:sales");
			...
			$edit->execute();
			//SAVE
			$save	= $conn->prepare("INSERT INTO pembayaran_faktur_c VALUES(:id, :sales, :bank, :norek, :nama, :bayar, :unik, :tgl, :catat, :admin, :catat, :admin)");
			...
			$save->execute();
			//RIWAYAT
			$riwayat= $conn->query("INSERT INTO riwayat_c VALUES('', '$kode', 'Pembayaran Outlet', 'Create', '', '$catat', '$admin')");
		...
		case "update":
			...
			$unik	= "pfaktur".time().".".$fofe;
			//SUMMARY
			$mbayar	= $conn->prepare("SELECT SUM(jumlah_pfk) AS total FROM pembayaran_faktur_c WHERE id_pfk!=:kode");
			$mbayar->bindParam(':kode', $kode, PDO::PARAM_STR);
			$mbayar->execute();
			$hbayar	= $mbayar->fetch(PDO::FETCH_ASSOC);
			//CEK DATA
			$read	= $conn->prepare("SELECT A.id_tfk, A.file_pfk, A.jumlah_pfk, B.total_tfk FROM pembayaran_faktur_c AS A LEFT JOIN transaksi_faktur_c AS B ON A.id_tfk=B.id_tfk WHERE A.id_pfk=:kode");
			...
			if($bayar!=$view['jumlah_pfk']){
				$edit	= $conn->prepare("UPDATE transaksi_faktur_c SET status_tfk=:status, updated_at=:catat, updated_by=:admin WHERE id_tfk=:kode");
			...
			if(!empty($loga)){
				...
				$file->uploadfoto('pfaktur', 'foto1', $unik, 400, 500);
				$edit	= $conn->prepare("UPDATE pembayaran_faktur_c SET file_pfk=:unik, updated_at=:catat, updated_by=:admin WHERE id_pfk=:kode");
			...
			//EDIT
			$edit	= $conn->prepare("UPDATE pembayaran_faktur_c SET bank_pfk=:bank, norek_pfk=:norek, anam_pfk=:nama, jumlah_pfk=:bayar, tgl_pfk=:tgl, updated_at=:catat, updated_by=:admin WHERE id_pfk=:kode");
			$edit->bindParam(":kode", $kode, PDO::PARAM_STR);
			...
			//RIWAYAT
			$riwayat= $conn->query("INSERT INTO riwayat_c VALUES('', '$kode', 'Pembayaran Outlet', 'UPDATE', '', '$catat', '$admin')");
			echo($hasil);
		break;
		case "delete":
			$kode	= $secu->injection($_POST['keycode']);
			...
			//DELETE
			unlink("../../berkas/pfaktur/$view[file_pfk]");
			$delete	= $conn->prepare("DELETE FROM pembayaran_faktur_c WHERE id_pfk=:kode");
			$delete->bindParam(":kode", $kode, PDO::PARAM_STR);
			$delete->execute();
			//RIWAYAT
			$riwayat= $conn->query("INSERT INTO riwayat_c VALUES('', '$kode', 'Pembayaran Outlet', 'Delete', '', '$catat', '$admin')");
