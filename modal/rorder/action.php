			...
			case 'input':
				...
				if($view['proses_tor']==='Open'){
					...
					while($no<$jum){
						...
						if(!empty($jumlah)){
							$save	= $conn->prepare("INSERT INTO transaksi_receivedetail VALUES(:id, :kode, :produk, :bcode, :tbcode, :jumlah, :harga, :diskon, :total, :catat, :admin, :catat, :admin)");
					...
					//STOK
					$save	= $conn->prepare("INSERT INTO produk_stokdetail SELECT '', id_trd, id_pro, bcode_trd, tbcode_trd, :tgltrm, jumlah_trd, 0, jumlah_trd, created_at, created_by, updated_at, updated_by FROM transaksi_receivedetail WHERE id_tre=:kode");
					$save->bindParam(':kode', $kode, PDO::PARAM_STR);
