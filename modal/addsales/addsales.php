    ...
    <div class="modal-body">
        <div class="row">
			...
			<table id="example1" class="tabelgetdata">
				...
				<?php
					...
					$qMaster = "SELECT
								...
								LEFT JOIN satuan_produk AS E ON
									B.id_spr = E.id_spr
								LEFT JOIN produk_diskon AS F ON
									B.id_pro = F.id_pro
								WHERE
									$notin
									AND A.sisa_psd>0
									AND C.status_phg =:status
									AND F.id_out =:mitra
								GROUP BY
								A.id_psd,
									A.no_bcode,
				...
				?>
                	<tr onClick="<?php echo("getproductsales('$nomor', '$hasil[id_psd]', '$hasil[id_pro]', '$hasil[nama_pro]', '$hasil[kode_pro]', '$hasil[harga_phg]', '$hasil[berat_pro]', '$hasil[nama_kpr]', '$hasil[satuan_kpr]', '$hasil[nama_spr]', '$hasil[no_bcode]', '$hasil[tgl_expired]', '$hasil[sisa_psd]', '$hasil[persen_pds]', '$taxStatus')"); ?>">
