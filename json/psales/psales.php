<?php
	...
	if($valid==false){
		...
		$tabel	= '';
		$no		= $mulai;
		$jumlah	= $conn->query("SELECT COUNT(A.id_tfk) AS total FROM transaksi_faktur_c AS A LEFT JOIN outlet AS C ON A.id_out=C.id_out WHERE A.kode_tfk LIKE '%$cari%' OR C.nama_out LIKE '%$cari%'")->fetch(PDO::FETCH_ASSOC);
		$master	= $conn->prepare("SELECT A.id_tfk, A.kode_tfk, A.tgl_tfk, A.tgl_limit, A.total_tfk, A.status_tfk, SUM(B.jumlah_pfk) AS bayar, C.nama_out FROM transaksi_faktur_c AS A LEFT JOIN pembayaran_faktur_c AS B ON A.id_tfk=B.id_tfk LEFT JOIN outlet AS C ON A.id_out=C.id_out WHERE A.kode_tfk LIKE '%$cari%' OR C.nama_out LIKE '%$cari%' GROUP BY A.id_tfk ORDER BY A.tgl_tfk DESC LIMIT :mulai, :maxi");
