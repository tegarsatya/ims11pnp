<?php
	...
	if($valid==false){
		...
		$tabel	= '';
		$no		= 1;
		$master	= $conn->prepare("SELECT C.nama_out, C.total FROM(SELECT A.nama_out, (SELECT SUM(B.total_tfk) FROM transaksi_faktur_c AS B WHERE B.id_out=A.id_out $tgl1 $tgl2) AS total FROM outlet AS A GROUP BY A.id_out) AS C ORDER BY C.total DESC LIMIT :rank");
