<?php
	...
	if($valid==false){
		...
		$master	= $conn->prepare("SELECT A.id_tkf_t, A.tanggal_tkf, A.status, A.note, B.kode_tfk, B.tgl_tfk, C.nama_out FROM tuker_faktur_c AS A INNER JOIN transaksi_faktur_c AS B ON A.id_tfk=B.id_tfk INNER JOIN outlet AS C ON A.id_kot=C.id_out WHERE (C.nama_out LIKE '%$cari%' OR C.nama_out LIKE '%$cari%') ORDER BY C.nama_out DESC LIMIT :mulai, :maxi");
