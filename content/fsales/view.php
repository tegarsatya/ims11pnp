...
<?php
	...
	$read	= $conn->prepare("SELECT A.kode_tsl, A.subtot_tsl, A.ppn_tsl, A.total_tsl, A.tgl_tsl, B.nama_out FROM transaksi_sales_c AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out WHERE A.id_tsl=:kode");
        ...
        <div class="row row-sm">
            <div class="col-sm-12">
				...
				<table class="table table-bordered">
                    ...
                    <tbody>
                    <?php
						$master	= $conn->prepare("SELECT A.jumlah_tsd, A.harga_tsd, A.diskon_tsd, A.total_tsd, B.kode_pro, B.nama_pro, B.berat_pro, C.nama_kpr, C.satuan_kpr, D.nama_spr FROM transaksi_salesdetail_c AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk AS C ON B.id_kpr=C.id_kpr LEFT JOIN satuan_produk AS D ON B.id_spr=D.id_spr WHERE A.id_tsl=:kode");
