...
<?php
	...
	$sumary	= $conn->query("SELECT SUM(total_tfk) AS total FROM transaksi_faktur_c WHERE YEAR(tgl_tfk)='$tahun'")->fetch(PDO::FETCH_ASSOC);
	...
	$viewout= $conn->query("SELECT COUNT(id_tfk) AS total FROM transaksi_faktur_c WHERE status_tfk!='Lunas' AND (TIMESTAMPDIFF(DAY, '$tanggal', tgl_limit)<=".$data->sistem('limit_outlet')." OR TIMESTAMPDIFF(DAY, tgl_limit, '$tanggal')>0)")->fetch(PDO::FETCH_ASSOC);
            ...
            <div class="block">
                ...
                <div class="block-content">
                    <table class="table table-striped table-vcenter">
						...
						<?php
							$nomor	= 1;
							$master	= $conn->prepare("SELECT C.nama_out, C.total FROM(SELECT B.nama_out, SUM(A.total_tfk) AS total FROM transaksi_faktur_c AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out WHERE LEFT(tgl_tfk, 7)=:bulan2 GROUP BY B.id_out) AS C ORDER BY C.total DESC LIMIT 10");
            ...
            <div class="block">
                ...
                <div class="block-content">
                    <table class="table table-striped table-vcenter">
						...
						<?php
							$nomor	= 1;
							$master	= $conn->prepare("SELECT C.nama_out, C.total FROM(SELECT B.nama_out, SUM(A.total_tfk) AS total FROM transaksi_faktur_c AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out WHERE LEFT(tgl_tfk, 7)=:bulan1 GROUP BY B.id_out) AS C ORDER BY C.total DESC LIMIT 10");
            ...
            <div class="block">
                ...
                <div class="block-content">
                    <table class="table table-striped table-vcenter">
						...
						<?php
							$nomor	= 1;
							$master	= $conn->prepare("SELECT C.ofcode_out, C.total FROM(SELECT B.ofcode_out, SUM(A.subtot_tfk) AS total FROM transaksi_faktur_c AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out WHERE LEFT(tgl_tfk, 7)=:bulan2 GROUP BY B.ofcode_out) AS C ORDER BY C.total DESC LIMIT 10 ");
							$master->bindParam(':bulan2', $bulan2, PDO::PARAM_STR);
							$master->execute();
							while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
						?>
            ...
            <div class="block">
                ...
                <div class="block-content">
                    <table class="table table-striped table-vcenter">
						...
						<?php
							$nomor	= 1;
							$master	= $conn->prepare("SELECT C.ofcode_out, C.total FROM(SELECT B.ofcode_out, SUM(A.subtot_tfk) AS total FROM transaksi_faktur_c AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out WHERE LEFT(tgl_tfk, 7)=:bulan1 GROUP BY B.ofcode_out) AS C ORDER BY C.total DESC LIMIT 10");
							$master->bindParam(':bulan1', $bulan1, PDO::PARAM_STR);
							$master->execute();
							while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
						?>
