...
<?php
    ...
    $read   = $conn->prepare("SELECT A.kode_tfk, A.tgl_tfk, A.tgl_limit, A.po_tfk, TIMESTAMPDIFF(DAY, A.tgl_tfk, A.tgl_limit) AS jarak, B.nama_out, B.npwp_out, C.pengiriman_ola FROM transaksi_faktur_c AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat AS C ON B.id_out=C.id_out WHERE A.id_tfk=:kode");
    ...
    <body>
        ...
        <table class="tabel">
            ...
            <?php
                ...
                $total  = 0;
                $stotal = 0;
                $master = $conn->prepare("SELECT A.jumlah_tfd, A.harga_tfd, A.diskon_tfd, B.no_bcode, B.tgl_expired, C.kode_pro, C.nama_pro, C.berat_pro, D.nama_kpr, E.nama_spr FROM transaksi_fakturdetail_c AS A LEFT JOIN produk_stokdetail AS B ON A.id_psd=B.id_psd LEFT JOIN produk AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk AS D ON C.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON C.id_spr=E.id_spr WHERE A.id_tfk=:kode");
        ...
        <div style="width:100%;">
            ...
            <div style="width:45%; display:inline-block; float:right;">
                ...
                <div style="min-height:30px; height:auto; border:solid 1px #666666; text-align:center; font-weight:bold; margin-top:10px; padding:5px; font-size:12px;">NB : Dokumen Ini Di Buat Secara Elektronik, Sehingga Tidak Membutuhkan Tanda Tangan </div>
                ...
                <div style="height:auto; border:solid 1px #666666; text-align:left; margin-top:10px; padding:5px; font-size:10px;">
                    ...
                    <div style="margin-top:5px;">Pembayaran Dapat Dilakukan Dengan Melakukan Transfer Ke :</div>
                    <div style="margin-left:15px; margin-top:5px;">BANK BCA </div>
                    <div style="margin-left:15px; margin-top:5px;">6560294997</div>
                    <div style="margin-left:15px; margin-top:5px;">An. Mohammad Dimas Tri</div>
                </div>
            </div>
        </div>
