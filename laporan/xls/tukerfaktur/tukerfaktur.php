...
?>
    ...
    <body>
    	...
    	<table border="1">
			...
			<?php
		$nomor		= $mulai;
// 		$jumlah	= $conn->query("SELECT COUNT(A.id_tkf_t) AS total FROM tuker_faktur_c AS A WHERE (A.id_tfk LIKE '%$cari%' OR A.id_tfk LIKE '%$cari%')")->fetch(PDO::FETCH_ASSOC);
		$master	= $conn->prepare("SELECT A.id_tkf_t, A.tanggal_tkf, A.status,A.note, B.kode_tfk, B.tgl_tfk, C.nama_out FROM tuker_faktur_c AS A INNER JOIN transaksi_faktur_c AS B ON A.id_tfk=B.id_tfk INNER JOIN outlet AS C ON A.id_kot=C.id_out ");
