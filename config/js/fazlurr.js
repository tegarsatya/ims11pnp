...
function addsales(nomor, outlet, taxStatus){
	...
	$.ajax({
		...
		dataType: "text",
		cache	: false,
		data	: { "m" : mitra, "x" : nomor, "y" : cart, 'taxStatus': taxStatus },
		success	: function(data){ $(".modal-content").html(data); }
	});
}
...
function addmaximal(menu, outlet, maksimal){
	...
	if(mitra==''){
	...
	} else if (menu == 'addsales' && !['P','NP'].includes(taxStatus)) {
		swal("Maaf!", "Pilih ketentuan pajak terlebh dahulu...", "warning");
	} else {
		if(parseInt(total)>=parseInt(maksimal)){
			...
			$.ajax({
				...
				data	: { "jumlah" : jumlah, "diskon" : diskon, "taxStatus": taxStatus },
				success	: function(data){
					$("#pilihoutlet").remove();
...
function getproductsales(nomor, kode, produk, nama, code, harga, berat, kategori, satuanqty, satuan, bcode, tgled, stok, diskon, taxStatus){
	var jumlah	= $("#pjumlah"+nomor).val(),
		jumlah	= jumlah=='' ? 1 : jumlah;
	//var diskon	= $("#pdiskon"+nomor).val(),
		//diskon	= diskon=='' ? 0 : diskon;
	...
	var total	= Math.round((parseInt(subtot) - ((subtot * diskon) / 100)), 0);
	var stotal	= parseInt(total) + parseInt(pstotal) - parseInt(ptotal);
	var ppn		= (taxStatus === 'P') ? Math.round(((stotal * 11) / 100), 0) : 0;
	var gtotal	= parseInt(stotal) + parseInt(ppn);
	$("#noproduct"+nomor).html('('+code+') '+nama);
...
function hitungsales(nomor,taxStatus){
...
function jumlahsales(nomor,taxStatus){
...
function showProductTransfer(nomor, kode, produk, nama, code, harga, berat, kategori, satuanqty, satuan, bcode, tgled, stok) {
	$("#noproduct"+nomor).html('('+code+') '+nama);
	$("#product"+nomor).val(produk);
...
function formApprovalTransferStokModal() {
	...
	$.ajax({
		...
		async		: true,
		dataType	: "text",
		data		: {"kode": kode, "approval": approval, "id": id, "type": type, "id_app_from": id_app_from, "id_app_to": id_app_to},
		success: function(data){
			var json	= JSON.parse(data);
			if(json.result=="Success"){
...
function taxStatusChange() {
	$("#dataaddsales tr").remove();
	$("#dataaddsales").html('<tr id="pilihoutlet"><td colspan="10">Pilih outlet dulu...</td></tr>');
	$("#minorder").val('');
	$("#diskon1").val('');
	$("#diskon2").val('');
	$("#cartaddsales").val('');
	...
	$("#pppn").val('0');
	$("#pgtotal").val('0');
	var taxStatus	= $('input[name="taxStatus"]:checked').val();
	if (taxStatus=='P') {
		$("#taxLabel").html("PPN (11%)");
	} else {
		$("#taxLabel").html("PPN (0%)");
	}
}
