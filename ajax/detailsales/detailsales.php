<?php
	...
	$total	= 0;
	$active	= 'Active';
	$master	= $conn->prepare("SELECT A.id_psd, A.jumlah_tsd, A.harga_tsd, A.diskon_tsd, A.total_tsd, B.id_pro, B.kode_pro, B.nama_pro, B.berat_pro, C.nama_kpr, C.satuan_kpr, D.nama_spr, E.no_bcode, E.tgl_expired FROM transaksi_salesdetail_c AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk AS C ON B.id_kpr=C.id_kpr LEFT JOIN satuan_produk AS D ON B.id_spr=D.id_spr LEFT JOIN produk_stokdetail AS E ON A.id_psd=E.id_psd WHERE A.id_tsl=:kode");
